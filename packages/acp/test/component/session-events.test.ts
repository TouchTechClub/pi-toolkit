...
test('PiAcpSession: emits ACP plan updates from pi todowrite tool results', async () => {
  const conn = new FakeAgentSideConnection()
  const proc = new FakePiRpcProcess()
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify([
            { content: 'implement undo', status: 'in_progress', priority: 'high' },
  ...
  assert.equal(conn.updates[0]!.update.sessionUpdate, 'plan')
  assert.deepEqual(conn.updates[0]!.update, {
    sessionUpdate: 'plan',
    entries: [
      { content: 'implement undo', priority: 'high', status: 'in_progress' },
      { content: 'write tests', priority: 'low', status: 'completed' },
    ],
  })
})
...
test('PiAcpSession: updates streamed bash tool title when execution args arrive', async () => {
  const conn = new FakeAgentSideConnection()
  const proc = new FakePiRpcProcess()
