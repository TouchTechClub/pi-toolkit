...
This repository is a maintained clone of [`svkozak/pi-acp`](https://github.com/svkozak/pi-acp). Thanks to [Sergii Kozak](https://github.com/svkozak) for creating the original adapter.
...
#### Using with `bunx` (no global install needed, always loads the latest version):
...
```bash
bun add -g @touchtechclub/pi-acp
```
...
```bash
bun install
bun run dev        # run from src via tsx
bun run build
bun run lint
bun run test
```
...
This clone is based on the original [`svkozak/pi-acp`](https://github.com/svkozak/pi-acp) by [Sergii Kozak](https://github.com/svkozak). Thank you to Sergii and upstream contributors for the foundation.
